...
const GLASS_CSS = `
  ...
  .dock-glass {
    ...
    box-shadow: 0 8px 32px rgba(0,0,0,0.45), 0 2px 8px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.22), inset 0 -1px 0 rgba(0,0,0,0.12);
  }
  .dock-glass::before {
  ...
  .add-app-btn:hover, .add-app-btn:focus-visible {
    ...
    transform: scale(1.04); outline: none; animation: addBtnPulse 2s ease-in-out infinite;
  }
  ...
  .add-app-btn:hover .add-app-btn-icon, .add-app-btn:focus-visible .add-app-btn-icon {
    background: rgba(99,179,255,0.18); border-color: rgba(99,179,255,0.4); color: rgba(99,179,255,0.9);
  }
  .add-app-btn-label {
    font-size: 12px; font-weight: 500; color: rgba(255,255,255,0.3); letter-spacing: 0.02em; transition: color 0.18s;
  }
  .add-app-btn:hover .add-app-btn-label, .add-app-btn:focus-visible .add-app-btn-label { color: rgba(99,179,255,0.75); }
`;
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      setFocus((prev) => {
        ...
        const libMax = libraryApps.length;
        ...
        switch (e.key) {
          case "ArrowRight": {
            ...
            return {
              area,
              index: Math.min(index + 1, area === "dock" ? dockMax : libMax),
            };
          }
          case "ArrowLeft": {
            e.preventDefault();
          ...
          case "ArrowDown": {
            e.preventDefault();
            if (area === "dock")
              return { area: "library", index: Math.min(index, libMax) };
            const next = index + cols;
            return next <= libMax
              ? { area, index: Math.min(next, libMax) }
              : prev;
          }
          case "Enter": {
            e.preventDefault();
              ...
              {dockApps.map((entry, i) => (
                <div
                  ...
                  style={{
                    ...
                    outlineOffset: "4px",
                    borderRadius: "18px",
                    transition: "opacity 0.15s, outline 0.08s, transform 0.15s",
                ...
                {libraryApps.map((entry, i) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: "14px",
                      transition:
                        "opacity 0.15s, outline 0.08s, transform 0.2s cubic-bezier(0.34,1.56,0.64,1)",
                      transform:
